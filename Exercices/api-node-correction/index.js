const http = require('http');
const { parse } = require('url');

// Données en mémoire
// Imaginer qu'il s'agit d'une base de données 
// Chaque ligne de mon tableau est une entrée dans une table de ma base de données par exemple 

let todos = [
  { id: 1, title: 'Acheter du lait', completed: false },
  { id: 2, title: 'Sortir le chien', completed: true },
  { id: 3, title: 'Coder une API', completed: false },
];
// id random pour chaque nouvelle tâche
let nextId = Math.floor(Math.random() * 1000000);

// A chaque fois que l'on veut envoyer une réponse JSON, on utilise cette fonction
const sendJson = (res, status, payload) => {
  res.writeHead(status, { 'Content-Type': 'application/json' });
  res.end(JSON.stringify(payload));
};

// La 404 en fin de chaine de requête
const notFound = (res) => sendJson(res, 404, { error: 'Not found' });

// Permet de lire le corps de la requête et de le parser en JSON
const parseBody = (req) =>
  // On crée une promesse car la lecture des données est asynchrone.
  new Promise((resolve) => {
    let data = '';
    // À chaque fois qu’un morceau du body arrive, on l’ajoute à data. HTTP arrive en paquets (streams), jamais tout d’un coup.
    req.on('data', (chunk) => {
      // Tout en format texte
      data += chunk.toString();
    });
    // Quand tout le body est arrivé, on parse le texte en JSON et on résout la promesse.
    req.on('end', () => {
      resolve(JSON.parse(data));
    });
  });

// Création de serveur HTTP
const server = http.createServer(async (req, res) => {

  // Récupérer l'url
  const { pathname } = parse(req.url);
  // Récupérer la méthode (sois GET, POST, PUT, DELETE, etc.)
  const method = req.method.toUpperCase();

  // Toutes les routes alternatives
  // GET /api/todos
  if (pathname === '/api/todos' && method === 'GET') {
    return sendJson(res, 200, todos);
  }
  // Gestion de l'index après la route /api/todos/
  if (pathname?.startsWith('/api/todos/')) {
    // On récupère la partie après la route /api/todos/
    const id = Number(pathname.split('/')[3]);
    if (Number.isNaN(id)) return notFound(res);
    // On cherche l'index de la tâche dans le tableau
    const index = todos.findIndex((t) => t.id === id);
    
    if (index === -1) return notFound(res);

    // GET /api/todos/id
    if (method === 'GET') {
      return sendJson(res, 200, todos[index]);
    }

    // DELETE /api/todos/id
    if (method === 'DELETE') {
      todos.splice(index, 1)[0];
      return sendJson(res, 204);
    }

    // PUT /api/todos/id
    if (method === 'PUT') {
      try {
        const body = await parseBody(req);
        const updated = { ...todos[index], ...body, id };
        todos[index] = updated;
        // On envoie la tâche mise à jour
        return sendJson(res, 200, updated);
      } catch (err) {
        return sendJson(res, 400, { error: err.message });
      }
    }
  }

  // POST /api/todos
  if (pathname === '/api/todos' && method === 'POST') {

    try {
      const body = await parseBody(req);
      // gestion simple des erreurs pour vérifier qu'il y a au moins un titre
      if (!body.title) {
        return sendJson(res, 400, { error: 'title requis' });
      }
      // On crée la tâche avec l'id suivant et les données du body
      const created = { id: nextId++, title: body.title, completed: !!body.completed };
      // On ajoute la tâche au tableau
      todos.push(created);
      // On envoie la tâche créée
      return sendJson(res, 201, created);
    } catch (err) {
      return sendJson(res, 400, { error: err.message });
    }
  }
  // Route fallback
  notFound(res);
});

const PORT = 3000;
server.listen(PORT, () => {
  console.log(`API démarrée sur http://localhost:${PORT}`);
});


