...
// Elle reçoit req, res, pathname, method et un objet avec les données partagées
module.exports = async function articlesRouter(req, res, pathname, method, { articles, nextId, saveArticles }) {
  
  // Route GET /articles - liste tous les articles
  if (pathname === '/articles' && method === 'GET') {
    listArticles(res, articles);
    return true;
  }

  // Route POST /articles - crée un nouvel article
  if (pathname === '/articles' && method === 'POST') {
    await createArticle(req, res, articles, nextId, saveArticles);
    return true;
  }
  ...
  if (pathname.startsWith('/articles/')) {
    ...
    if (method === 'GET') {
      getArticle(res, id, articles);
      return true;
    }

    // Route PUT /articles/:id - met à jour un article
    if (method === 'PUT') {
      await updateArticle(req, res, id, articles, saveArticles);
      return true;
    }

    // Route DELETE /articles/:id - supprime un article
    if (method === 'DELETE') {
      deleteArticle(res, id, articles, saveArticles);
      return true;
    }
  }
